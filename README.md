...
Mouth Monitor 面向那些**因为习惯、疲劳或嘴唇闭合能力不足，经常在无意识中张嘴**的人。它使用普通摄像头观察嘴唇开合状态；当嘴唇持续张开超过设定时间时，页面会变红并播放系统提示音，让提醒发生在习惯形成的当下，而不是等你偶然从镜子里发现。

它不是医疗设备，也不会声称自己能够“诊断口呼吸”。摄像头只能看到嘴唇是否张开，无法判断空气究竟经过鼻腔还是口腔。因此，本项目解决的是一个更明确的问题：

> **当我长时间无意识张嘴时，及时提醒我尝试闭合嘴唇。**

![Mouth Monitor 控制台](docs/screenshot.png)

## 为什么需要个人校准

不同人的唇形、摄像头角度、拍摄距离和自然闭合状态差异很大。Mouth Monitor 不使用一套固定阈值要求所有人“长得一样”，而是让用户分别采集：
...
- 摄像头画面只在当前电脑内存中处理。
- 不需要账号、云服务、API Key 或网络推理接口。
- 不保存图片、视频和完整面部关键点。
- 只有设置和个人校准结果会写入本地配置文件。
...
- 判断空气是否真正经过口腔；
- 诊断口呼吸、睡眠呼吸问题、鼻腔阻塞或颌面问题；
- 在睡眠、多人画面、侧脸、严重遮挡或光线很差的环境下使用；
- 替代医生、牙医、正畸医生或其他专业人员的判断。
...
进入和退出使用不同阈值，避免比例在边界附近波动时状态反复切换。持续时间使用单调时钟计算，而不是按视频帧数量估算。

## 隐私设计

- FastAPI 服务只绑定本机回环地址，不对局域网开放；
- 摄像头帧仅用于实时推理和当前页面预览；
- 程序不创建图片、视频或人脸关键点历史文件；
- `%LOCALAPPDATA%\MouthMonitor\config.json` 只保存设置与校准后的数值；
- 停止或暂停监控时会释放摄像头；
- 当前版本没有账号、遥测、广告、云端 API 或远程数据库。

## 已知限制

- 嘴唇张开不等于正在口呼吸，鼻呼吸时也可能张嘴；
- 嘴唇闭合也不能排除其他呼吸或睡眠问题；
- 光线、遮挡、侧脸、说话、进食和摄像头位置会影响识别结果；
- 当前说话过滤基于嘴唇比例波动，是启发式规则，不是语音活动识别；
- 当前版本需要保持应用服务运行，不支持静默自启或完整系统托盘后台模式；
- Windows 安装包尚未代码签名，系统可能显示未知发布者提示。

如果你长期无法自然闭合嘴唇，或伴随鼻塞、睡眠、呼吸、牙齿及颌面问题，请咨询专业医生。
